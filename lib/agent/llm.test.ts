...
export async function llmContractTests() {
  ...
  await withEnv(async () => { const result = await callChatCompletions(request, async (_url, init) => new Promise((_resolve, reject) => init?.signal?.addEventListener('abort', () => reject(new DOMException('timeout', 'AbortError'))))); assert.equal('error' in result, true); });
}
